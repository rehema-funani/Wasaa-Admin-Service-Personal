...
const AuditLogDetailsPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchLogDetails = async () => {
            ...
            try {
                ...
                setError('Failed to load audit log details. The log may not exist or you may not have permission to view it.');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const getEventTypeBadge = (eventType: string) => {
        return (
            <Badge
                variant={
                    eventType.includes('create') ? 'success' :
                        eventType.includes('update') ? 'primary' :
                            eventType.includes('delete') ? 'danger' :
                                eventType.includes('login') ? 'info' :
                                    eventType.includes('fetch') ? 'default' :
                                        'default'
                }
                size="md"
            >
    ...
    if (error || !log) {
        return (
            <div className="p-6">
                ...
                <Card className="bg-red-50 border border-red-200 text-red-700 p-6 flex items-start">
                    <AlertTriangle size={24} className="mr-4 flex-shrink-0" />
                    <div>
                        <h2 className="text-lg font-medium mb-2">Error Loading Audit Log</h2>
                        <p>{error || "Something went wrong while loading the audit log details."}</p>
                        <Button
                            variant="outline"
    ...
    return (
        <div className="w-full py-6 mx-auto">
            ...
            <Card className="mb-6 bg-white/70 backdrop-blur-md border border-white/50 overflow-hidden">
                <div className="border-b border-gray-100 p-4 flex flex-wrap items-center justify-between gap-4">
                    <div className="flex items-center space-x-3">
                        <div
                            className={`w-2 h-8 rounded-full ${log.is_successful ? 'bg-green-500' : 'bg-red-500'}`}
                        ></div>
                        <div>
                            <h2 className="text-lg font-medium text-gray-900">{log.event_description}</h2>
                ...
                <div className="p-5 grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="flex items-start space-x-3">
                        ...
                        <div>
                            <h3 className="text-sm font-medium text-gray-900">{getUsernameDisplay(log)}</h3>
                            <p className="text-sm text-gray-500">{getUserEmailDisplay(log)}</p>
                            <p className="text-xs text-gray-400 mt-1">User ID: {log.user_id}</p>
                        </div>
                    </div>

                    <div className="flex items-start space-x-3">
                        <Database size={18} className="text-gray-400 flex-shrink-0 mt-0.5" />
                        <div>
                            <h3 className="text-sm font-medium text-gray-900">{log.entity_affected}</h3>
                            <p className="text-sm text-gray-500 truncate max-w-[250px]">{log.entity_id || 'No entity ID'}</p>
                            <p className="text-xs text-gray-400 mt-1">Method: {log.http_method}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-start space-x-3">
                        ...
                        <div>
                            <h3 className="text-sm font-medium text-gray-900">{log.ip_address.replace('::ffff:', '')}</h3>
                            <p className="text-sm text-gray-500">{log.location || 'Unknown location'}</p>
                            <p className="text-xs text-gray-400 mt-1">Execution: {log.execution_time}ms</p>
                        </div>
                    </div>
                </div>
            </Card>
            ...
            <div className="space-y-6">
                {activeTab === 'overview' && (
                    <>
                        <Card className="p-5 bg-white/70 backdrop-blur-md border border-white/50">
                ...
                {activeTab === 'request' && (
                    <>
                        <Card className="p-5 bg-white/70 backdrop-blur-md border border-white/50">
                            ...
                            <h4 className="text-sm font-medium text-gray-600 mb-2">Request Body</h4>
                            ...
                            <div className="mb-4">
                                <div className="flex items-center gap-2 mb-4">
                                    ...
                                    <div className="ml-4 text-sm text-gray-600">
                                        Status Code: <span className="font-medium">{log.status_code}</span>
                                    </div>
                                </div>

                                <h4 className="text-sm font-medium text-gray-600 mb-2">Response Body</h4>
                ...
                {activeTab === 'device' && (
                    <Card className="p-5 bg-white/70 backdrop-blur-md border border-white/50">
                        <h3 className="text-lg font-medium text-gray-900 mb-4">Device & Location Information</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <InfoItem
                                label="IP Address"
                                value={log.ip_address.replace('::ffff:', '')}
