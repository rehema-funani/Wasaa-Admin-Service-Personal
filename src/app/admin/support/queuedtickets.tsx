...
const PriorityBadge = ({ priority }) => {
  ...
  return (
    <div className={`px-2 py-0.5 text-xs font-medium rounded-full border flex items-center ${config[priority].color}`}>
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <div className={`px-2 py-0.5 text-xs font-medium rounded-full border flex items-center ${statusConfig.color}`}>
...
const CategoryBadge = ({ category }) => {
  ...
  return (
    <div className={`px-2 py-0.5 text-xs font-medium rounded-full border ${config.color}`}>
    ...
    const fetchTickets = async () => {
      ...
      try {
        ...
        const dummyTickets: Ticket[] = Array.from({ length: 15 }, (_, i) => {
          ...
          // Generate a random update date between created date and now
          ...
          // Generate a random due date between now and 7 days in the future
          const dueDate = new Date();
          dueDate.setDate(dueDate.getDate() + Math.floor(Math.random() * 7));

          // Status should be "new" for tickets in the queue, but we'll have a mix for demonstration
          const status =
            Math.random() < 0.7
              ? "new"
        ...
        // Sort by created date (newest first) for initial view
        dummyTickets.sort(
          (a, b) =>
