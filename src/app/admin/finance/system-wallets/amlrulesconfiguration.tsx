...
import { AMLRule, RuleAction, RuleCondition, RuleEffectiveness, RuleParameter } from '../../../../types/compliance';
...
const AMLRulesConfiguration: React.FC = () => {
    ...
    const [modalType, setModalType] = useState<'edit' | 'add' | 'delete' | 'test' | null>(null);
            ...
            <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white p-6 rounded-xl shadow-xl mb-8">
                <div className="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <p className="text-blue-100 mt-1">Configure and manage anti-money laundering rules</p>
                    </div>
                    <div className="flex items-center gap-3">
                        ...
                        <button
                            className="flex items-center gap-1.5 px-4 py-2 bg-blue-500/20 text-white rounded-lg hover:bg-blue-500/30 transition-colors text-sm font-medium backdrop-blur-sm"
                        >
                            <Download size={16} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="bg-white/10 backdrop-blur-md rounded-xl p-5 border border-white/20">
                        ...
                        <p className="text-3xl font-bold">
                            {Object.keys(rules.reduce((acc, rule) => ({ ...acc, [rule.category]: true }), {})).length}
                        </p>
                        <div className="flex flex-wrap gap-2 mt-3">
                            <span className="text-xs bg-blue-400/20 backdrop-blur-sm text-blue-100 px-2 py-1 rounded-full border border-blue-400/30">
                ...
                <div className="p-4 border-b border-slate-100 flex flex-wrap md:flex-nowrap gap-3 items-center justify-between bg-white">
                    ...
                    <div className="flex items-center flex-wrap gap-2">
                        ...
                        <div className="flex items-center ml-2">
                            ...
                            />
                            <label htmlFor="showSystemRules" className="ml-2 text-sm text-slate-600">
                                Show System Rules
                            </label>
                        </div>
                    </div>
                ...
                {isLoading ? (
                    <div className="p-10">
                        <div className="flex flex-col justify-center items-center">
                            ...
                            <span className="text-slate-500 font-medium">Loading AML rules...</span>
                        </div>
                    </div>
                ) : filteredRules.length === 0 ? (
                    <div className="p-12 text-center">
                        <Settings size={48} className="mx-auto text-slate-300 mb-4" />
                        <h3 className="text-xl font-medium text-slate-700 mb-2">No rules found</h3>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-slate-200">
                            <thead className="bg-gradient-to-r from-slate-50 to-white">
                                <tr>
                                    <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Rule</th>
                                    <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Type</th>
                                    <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Category</th>
                                    <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Status</th>
                                    <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Severity</th>
                                    <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Effectiveness</th>
                                    <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Updated</th>
                            ...
                            <tbody className="bg-white divide-y divide-slate-100">
                                {filteredRules.map((rule) => {
                                    ...
                                    return (
                                        <tr key={rule.id} className="hover:bg-slate-50 transition duration-150">
                                            <td className="px-4 py-4 whitespace-nowrap">
                                                <div className="flex items-center">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="text-xs text-slate-500 mt-0.5 max-w-[250px] truncate" title={rule.description}>
                                                            {rule.description}
                                                        </div>
                                                        {rule.isSystemRule && (
                                            ...
                                            <td className="px-4 py-4 whitespace-nowrap">
                                                <div className="flex items-center">
                                                    {getRuleTypeIcon(rule.ruleType)}
                                                    <span className="ml-2 text-sm text-slate-700">{formatRuleType(rule.ruleType)}</span>
                                                </div>
                                            </td>
                                            <td className="px-4 py-4 whitespace-nowrap">
                                                <div className="flex items-center">
                                                    {getCategoryIcon(rule.category)}
                                                    <span className="ml-2 text-sm text-slate-700">{formatCategory(rule.category)}</span>
                                                </div>
                                            </td>
                                            <td className="px-4 py-4 whitespace-nowrap">
                                                <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium shadow-sm ${getStatusColor(rule.status)}`}>
                                                    {getStatusIcon(rule.status)}
                                                    {rule.status.charAt(0).toUpperCase() + rule.status.slice(1)}
                                                </span>
                                            </td>
                                            <td className="px-4 py-4 whitespace-nowrap">
                                                <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium shadow-sm ${getSeverityColor(rule.severity)}`}>
                                                ...
                                                {effectiveness ? (
                                                    <div className="flex items-center">
                                                        <div className="mr-2 w-16 bg-slate-200 rounded-full h-2.5 overflow-hidden">
                                                            <div
                                                                className={`h-2.5 rounded-full ${effectiveness.efficiency >= 0.9 ? 'bg-gradient-to-r from-emerald-400 to-green-500' :
                                                                    effectiveness.efficiency >= 0.7 ? 'bg-gradient-to-r from-blue-400 to-indigo-500' :
                                                                        effectiveness.efficiency >= 0.5 ? 'bg-gradient-to-r from-amber-400 to-orange-500' :
