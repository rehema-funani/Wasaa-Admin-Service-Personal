...
const BlacklistWhitelistManager: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"blacklist" | "whitelist">(
    "blacklist"
  );
  const [searchQuery, setSearchQuery] = useState<string>("");
  const [entityTypeFilter, setEntityTypeFilter] = useState<
    "all" | "user" | "email" | "phone" | "ip_address" | "device_id" | "account"
  >("all");
  const [statusFilter, setStatusFilter] = useState<
    "all" | "pending" | "active" | "inactive" | "approved" | "rejected"
  >("all");
  ...
  const filteredEntries = entries.filter((entry) => {
    ...
    const matchesSearch =
      ...
      false ||
      entry.UserWallet.user_uuid
        ?.toLowerCase()
        .includes(searchQuery.toLowerCase()) ||
      entry.UserWallet.group_uuid
        .toLowerCase()
        .includes(searchQuery.toLowerCase());
  ...
  const handleAddEntry = async () => {
    try {
      ...
      showSuccess(
        `${
          activeTab === "blacklist" ? "Blacklist" : "Whitelist"
  ...
  const getModalTitle = () => {
    switch (modalType) {
      case "view":
        return `${
          selectedEntry?.type === "blacklist" ? "Blacklist" : "Whitelist"
        } Entry Details`;
      case "add":
        return `Add ${
          activeTab === "blacklist" ? "Blacklist" : "Whitelist"
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div>
              ...
              <p className="text-slate-600 mt-2">
                Advanced blacklist and whitelist management for financial
                compliance
              </p>
            </div>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-white/50 overflow-hidden">
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <p
                          ...
                          title={entry.reason || "No reason provided"}
                        >
          ...
          <div className="px-6 py-4 border-t border-slate-200/50 flex items-center justify-between bg-slate-50/50">
            <div className="text-sm text-slate-600">
              ...
              {entries.filter((e) => e.type === activeTab).length} entries
            </div>
            <div className="flex items-center gap-2">
              <button
                className="px-4 py-2 text-sm text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-50 transition-all"
                disabled
              >
                Previous
              </button>
            ...
            <div className="p-4 bg-red-50 rounded-lg border-l-4 border-red-400">
              <div className="flex items-center gap-3">
                ...
                <p className="text-sm font-medium text-red-800">
                  Are you sure you want to delete this {selectedEntry.type}{" "}
                  entry? This action cannot be undone and will be logged for
                  audit purposes.
                </p>
              </div>
            </div>
