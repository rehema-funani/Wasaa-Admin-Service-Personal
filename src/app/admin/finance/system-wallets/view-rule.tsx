...
const RuleDetailPage: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'parameters' | 'conditions' | 'actions' | 'history'>('overview');
        ...
        const fetchRuleDetails = async () => {
            ...
            try {
                if (location.state?.rule) {
                    ...
                    const effectiveness = mockRuleEffectiveness.find(item => item.ruleId === location.state.rule.id);
                ...
                setTimeout(() => {
                    ...
                    const foundEffectiveness = mockRuleEffectiveness.find(item => item.ruleId === ruleId);
    ...
    const renderParametersSection = () => {
        if (!rule?.parameters || rule.parameters.length === 0) {
            return <p className="text-sm text-slate-500 italic">No parameters defined</p>;
        }
    ...
    const renderConditionsSection = () => {
        if (!rule?.conditions || rule.conditions.length === 0) {
            return <p className="text-sm text-slate-500 italic">No conditions defined</p>;
        }

        return (
            <div className="space-y-3">
                {rule.conditions.map((condition, index) => (
                    <div key={condition.id} className="p-4 bg-white rounded-xl border border-slate-200 shadow-sm">
                        ...
                        <div className="text-sm p-3 bg-slate-50 rounded-lg border border-slate-100">
                            <span className="text-blue-600 font-medium">{condition.field}</span>
                            <span className="mx-1.5 text-slate-500">{condition.operator}</span>
    ...
    const getActionIcon = (type: string) => {
        switch (type) {
            case 'generate_alert': return <AlertTriangle className="w-4 h-4 text-amber-500" />;
            case 'block_transaction': return <XCircle className="w-4 h-4 text-rose-500" />;
            case 'escalate': return <AlertTriangle className="w-4 h-4 text-rose-500" />;
            case 'request_verification': return <CheckCircle2 className="w-4 h-4 text-blue-500" />;
            case 'notify': return <Info className="w-4 h-4 text-blue-500" />;
            case 'log': return <Terminal className="w-4 h-4 text-slate-500" />;
            case 'reduce_limits': return <DollarSign className="w-4 h-4 text-amber-500" />;
    ...
    const renderActionsSection = () => {
        if (!rule?.actions || rule.actions.length === 0) {
            return <p className="text-sm text-slate-500 italic">No actions defined</p>;
        }

        return (
            <div className="space-y-3">
                {rule.actions.map((action, index) => (
                    <div key={action.id} className="p-4 bg-white rounded-xl border border-slate-200 shadow-sm">
                        ...
                        <div className="text-sm text-slate-600">
                            {action.description && (
                                <div className="mb-2 text-sm text-slate-500">{action.description}</div>
                            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                        <h3 className="text-lg font-semibold text-slate-800 mb-4">Rule Description</h3>
                        <p className="text-slate-600 whitespace-pre-line">{rule.description || 'No description provided.'}</p>
                    </div>

                    <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                        <h3 className="text-lg font-semibold text-slate-800 mb-4">Conditions Summary</h3>
                        {rule.conditions && rule.conditions.length > 0 ? (
                                ...
                                {rule.conditions.map((condition, index) => (
                                    <div key={index} className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                                        <div className="text-sm">
                                            ...
                                            <span className="text-blue-600 font-medium">{condition.field}</span>
                                            <span className="mx-1 text-slate-500">{condition.operator}</span>
                                            <span className="text-amber-600 font-medium">
                            ...
                            <p className="text-slate-500 italic">No conditions defined for this rule.</p>
                        )}
                    </div>

                    <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                        <h3 className="text-lg font-semibold text-slate-800 mb-4">Actions Summary</h3>
                        {rule.actions && rule.actions.length > 0 ? (
                            ...
                            <p className="text-slate-500 italic">No actions defined for this rule.</p>
                        )}
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                        <h3 className="text-lg font-semibold text-slate-800 mb-4">Rule Properties</h3>
                        <div className="space-y-4">
                            <div>
                                ...
                                <div className="flex items-center">
                                    ...
                                    <span className="text-slate-700 font-medium">{formatRuleType(rule.ruleType)}</span>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="flex items-center">
                                    ...
                                    <span className="text-slate-700 font-medium">{formatCategory(rule.category)}</span>
                                </div>
                            </div>

                            <div>
                                <div className="text-sm text-slate-500 mb-1">Status</div>
                                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-medium shadow-sm ${getStatusColor(rule.status)}`}>
                            ...
                            <div>
                                ...
                                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-medium shadow-sm ${getSeverityColor(rule.severity)}`}>
                            ...
                            <div>
                                ...
                                <div className="text-slate-700 font-medium">v{rule.version.toFixed(1)}</div>
                            </div>

                            <div>
                                <div className="text-sm text-slate-500 mb-1">Rule ID</div>
                                <div className="flex items-center">
                                    <div className="text-slate-700 font-mono text-sm mr-2">{rule.id}</div>
                                    <button className="p-1 text-slate-400 hover:text-blue-500" title="Copy rule ID">
                                        <Copy size={14} />
                                    </button>
                                </div>
                            </div>
                    ...
                    {ruleEffectiveness && (
                        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                            <h3 className="text-lg font-semibold text-slate-800 mb-4">Effectiveness</h3>
                            <div className="space-y-4">
                                <div>
                                    <div className="text-sm text-slate-500 mb-2">Efficiency Rate</div>
                                    <div className="flex items-center">
                                        <div className="w-full bg-slate-100 rounded-full h-3 mr-3">
                                            <div
                                                className={`h-3 rounded-full ${ruleEffectiveness.efficiency >= 0.9 ? 'bg-gradient-to-r from-emerald-400 to-green-500' :
                                                        ruleEffectiveness.efficiency >= 0.7 ? 'bg-gradient-to-r from-blue-400 to-indigo-500' :
                                                            ruleEffectiveness.efficiency >= 0.5 ? 'bg-gradient-to-r from-amber-400 to-orange-500' :
                                                                'bg-gradient-to-r from-rose-400 to-red-500'
                                                    }`}
                                                style={{ width: `${ruleEffectiveness.efficiency * 100}%` }}
                                            ></div>
                                        </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-emerald-50 p-3 rounded-lg border border-emerald-100">
                                        <div className="text-xs text-emerald-600 mb-1">True Positives</div>
                                        <div className="text-lg font-bold text-emerald-700">{ruleEffectiveness.truePositives}</div>
                                    </div>

                                    <div className="bg-rose-50 p-3 rounded-lg border border-rose-100">
                                        <div className="text-xs text-rose-600 mb-1">False Positives</div>
                                        <div className="text-lg font-bold text-rose-700">{ruleEffectiveness.falsePositives}</div>
                                    </div>
                                </div>

                                <div>
                                    <div className="text-sm text-slate-500 mb-1">Last Triggered</div>
                                    <div className="text-slate-700">
                                        {ruleEffectiveness.lastTriggered ? formatDate(ruleEffectiveness.lastTriggered) : 'Never'}
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-slate-50 to-slate-100">
                <div className="flex flex-col items-center">
                    ...
                    <div className="text-slate-600 font-medium">Loading rule details...</div>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-slate-50 to-slate-100">
                <div className="flex flex-col items-center max-w-md text-center p-8 bg-white rounded-xl shadow-xl border border-slate-200">
                    ...
                    <h2 className="text-xl font-bold text-slate-800 mb-2">Rule Not Found</h2>
                    <p className="text-slate-600 mb-6">The rule you're looking for doesn't exist or you don't have permission to view it.</p>
                    <button
                        onClick={() => navigate(-1)}
                ...
                <div className="container mx-auto">
                    <div className="flex items-center mb-6">
                        ...
                        <div className="flex items-center gap-2">
                            {!rule.isSystemRule && (
                                <>
                                    <button
                                        onClick={() => navigate(`/compliance/aml/rules/${rule.id}/edit`, { state: { rule } })}
                                    ...
                                    <button
                                        onClick={() => navigate(`/compliance/aml/rules/${rule.id}/delete`, { state: { rule } })}
                            ...
                            <button
                                ...
                                className={`flex items-center gap-1.5 px-3 py-1.5 ${rule.status === 'active'
                            ...
                            >
                                ...
                                <span>{rule.status === 'active' ? 'Deactivate' : 'Activate'}</span>
                            </button>
                            <button
                                className="flex items-center gap-1.5 px-3 py-1.5 bg-white/10 hover:bg-white/20 backdrop-blur-sm rounded-lg transition-colors text-sm"
                                title="Export rule"
                    ...
                    <div className="flex flex-wrap gap-3 mb-4">
                        <div className="flex items-center bg-white/10 px-3 py-1 rounded-full backdrop-blur-sm">
                            {getRuleTypeIcon(rule.ruleType)}
                            <span className="ml-2 text-sm">{formatRuleType(rule.ruleType)}</span>
                        </div>
                        <div className="flex items-center bg-white/10 px-3 py-1 rounded-full backdrop-blur-sm">
                            {getCategoryIcon(rule.category)}
                            <span className="ml-2 text-sm">{formatCategory(rule.category)}</span>
                        </div>
                        <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm ${getStatusColor(rule.status)}`}>
                            {getStatusIcon(rule.status)}
                            {rule.status.charAt(0).toUpperCase() + rule.status.slice(1)}
                        </span>
                        <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm ${getSeverityColor(rule.severity)}`}>
            ...
            <div className="container mx-auto px-4">
                ...
                <div className="mb-8 border-b border-slate-200">
                    <div className="flex overflow-x-auto hide-scrollbar">
                        ...
                        <button
                            className={`px-5 py-3 text-sm font-medium ${activeTab === 'parameters'
                                    ? 'text-blue-600 border-b-2 border-blue-600'
                                    : 'text-slate-600 hover:text-blue-600'
                                }`}
                        ...
                        <button
                            className={`px-5 py-3 text-sm font-medium ${activeTab === 'conditions'
                                    ? 'text-blue-600 border-b-2 border-blue-600'
                                    : 'text-slate-600 hover:text-blue-600'
                                }`}
                        ...
                        <button
                            className={`px-5 py-3 text-sm font-medium ${activeTab === 'actions'
                                    ? 'text-blue-600 border-b-2 border-blue-600'
                                    : 'text-slate-600 hover:text-blue-600'
                                }`}
