...
const OtpVerificationPage = () => {
  ...
  const handleSubmit = async (e?: React.FormEvent, manualOtp?: string) => {
    ...
    if (!userId) {
      setErrors({
        general: "User ID is missing. Please go back to login page.",
      });
      return;
    }
    ...
    try {
      ...
      setErrors({
        general:
          formatErrorMessage(err) || "Verification failed. Please try again.",
      });
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      setErrors({
        general:
          formatErrorMessage(err) || "Failed to resend OTP. Please try again.",
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const handleOtpChange = (index: number, value: string) => {
    ...
    if (index === 5 && value) {
      ...
      const previousDigitsFilled = completeOtp
        .slice(0, 5)
        .every((digit) => digit !== "");
            ...
            <div className="relative z-10">
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="px-2 py-1 bg-teal-50 dark:bg-teal-900/30 text-xs font-medium text-teal-600 dark:text-teal-400 rounded-full">
                    Enhanced
                  </span>
                </div>
              ...
              >
                ...
                <div className="relative h-2.5 w-full bg-gray-100 dark:bg-gray-600 rounded-full mb-3">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: showSuccess ? "100%" : "75%" }}
                    transition={{
                      width: {
                        duration: showSuccess ? 0.5 : 1,
                        ease: "easeOut",
                      },
                      delay: 0.7,
                    }}
                  ...
                  <div className="absolute inset-0 flex items-center">
                    <div className="w-full flex justify-between px-1">
                      ...
                      <motion.div
                        ...
                        className="w-4 h-4 bg-white rounded-full border-2 border-blue-500 relative left-[-0.75rem] flex items-center justify-center"
                      >
                        {showSuccess && (
                          <motion.div
                            initial={{ scale: 0 }}
        ...
        >
          ...
          >
            ...
            <div>
                  ...
                  >
                    <div
                      className={`relative ${
                        ...
                        errors.otp
                          ? "border-red-300 dark:border-red-600 shadow-sm shadow-red-100 dark:shadow-red-900/20"
                          : otpValues[index]
                          ? "border-teal-300 dark:border-teal-600 shadow-sm shadow-teal-100 dark:shadow-teal-900/20"
                    ...
                    >
                      <input
                        ...
                        className={`w-full aspect-square text-center text-lg font-medium bg-transparent text-gray-900 dark:text-gray-100 focus:outline-none transition-all duration-200 disabled:text-gray-500 dark:disabled:text-gray-400`}
                      />
                      ...
                      {showSuccess && (
                        ...
                        >
                          <div
                            className={`absolute inset-0 bg-teal-50 dark:bg-teal-900/30 opacity-30 rounded-xl`}
                          ></div>
                        </motion.div>
                      )}
                        ...
                        animate={{
                          scaleX:
                            inputRefs.current[index] ===
                              document.activeElement && !showSuccess
                              ? 1
                              : 0,
                        }}
                        className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-teal-500 to-blue-500 origin-left"
                      />
                    </div>
                  </motion.div>
                ))}
              ...
              <p className="mt-3 text-xs text-gray-500 dark:text-gray-400 flex items-center">
                <svg
                  className="w-3.5 h-3.5 mr-1 text-gray-400 dark:text-gray-500"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className={`w-full flex justify-center items-center py-3.5 px-4 mt-4 border border-transparent rounded-xl text-sm font-medium text-white transition-all duration-200 ${
                showSuccess
                  ? "bg-teal-500 shadow-md shadow-teal-100 dark:shadow-teal-900/20"
                  : "bg-gradient-to-r from-teal-500 to-blue-500 hover:shadow-md"
              }`}
              disabled={isLoading || showSuccess}
            >
              {isLoading ? (
          ...
          <div className="mt-8">
            <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
              <div className="flex items-center justify-between">
                <div className="flex items-start">
                  <svg
                    className="w-5 h-5 text-gray-500 dark:text-gray-400 mt-0.5 mr-3"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                ...
                {canResend ? (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="px-3 py-1.5 text-sm font-medium rounded-lg text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/30 hover:bg-teal-100 dark:hover:bg-teal-900/50 transition-colors"
          ...
          <div className="mt-8 text-center">
            <button
              ...
              className="text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-teal-600 dark:hover:text-teal-400 transition-colors"
              disabled={showSuccess}
            >
              ← Back to login
            </button>
