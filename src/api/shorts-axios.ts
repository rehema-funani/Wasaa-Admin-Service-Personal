...
const baseURL = import.meta.env.VITE_API_URL || 'http://138.68.190.213:8079/api/';
...
export const shortaxios = axios.create({
  baseURL,
  headers: {
    'Content-Type': 'application/json',
    'Accept': 'application/json',
...
shortaxios.interceptors.response.use(
  (response) => {
    return response;
  },
  async (error) => {
